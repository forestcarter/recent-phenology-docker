	...
	for index,mydate in enumerate(datearray):
		...
		while (success==False and dlfailed==False):
			if totaltries>100 or (index==0 and int(mydate.yearjul[-7:]) < int(mostrecentdate[-7:])):
				...
				break
			print (str(datetime.now()))
			indexfile = os.path.join(staticpath,'{0}.html'.format(mydate.yearjul))                       
			...
			htmldl=['/usr/bin/wget', '-O', indexfile, '--no-proxy', '-t', '3', '-o', os.path.join(staticpath,'dlhtml.txt'), '--no-check-certificate', '-L', '--user='+username, '--password='+password, '--no-parent', '-A', 'US_eMAE_NDVI.'+mydate.year+'.*.QKM.*.zip',url1]
			print (htmldl) 
			...
			print("startingunzip")
			unzipCommand = "/usr/bin/unzip -o {0}.zip -d {0}".format(zipdestination)
			print(unzipCommand)
...
if len(targetdatearray)==len(minusarray):
	...
	for (ind, mydate) in enumerate(targetdatearray):
		...
		if ind==0:
			...
			savetodb =("raster2pgsql -s 4326 -t auto -I -C -M -F {0} public.{1} | PGPASSWORD={2} psql -U root -d ndvidb2").format("gdal_calc.tif",mydate.yearjul,pgpassword)
			print(savetodb)
			try:
